// ============================================================
// MOCK DATA — Datos de desarrollo
// Última actualización: Marzo 2026
// ============================================================

// ── CÓDIGOS ACTIVOS ────────────────────────────────────────

export const CODES_DATA = {
  "Wuthering Waves": {
    icon: "https://play-lh.googleusercontent.com/ohBD7vN6RUU6KoD4PJgwv5Bg3HvDIjM4DvHd7NlRxRFkMJGxSEipUjexdFYJf7g1RA",
    codes: [
      { code: "WUTHERINGGIFT",  rewards: "50 Astrite, 2 Premium Resonance Potion", expires: "31 Mar 2026" },
      { code: "WUTHERING2024",  rewards: "10,000 Shell Credits",                   expires: "31 Mar 2026" },
      { code: "WUWA333",        rewards: "60 Astrite, 1 Sealed Tube",              expires: "28 Mar 2026" },
      { code: "KUROLAUNCH",     rewards: "80 Astrite, 3 Hero EXP",                 expires: "29 Mar 2026" },
      { code: "WW2ANNI",        rewards: "50 Astrite",                             expires: "15 Abr 2026" },
      { code: "WAVEBB888",      rewards: "30 Astrite, 5,000 Shell Credits",        expires: "10 Abr 2026" },
    ],
  },

  "Genshin Impact": {
    icon: "https://play-lh.googleusercontent.com/L0Dn7FRoGO0Ocxb3MidlNpv_DMehL8xlEkVJcnb2MaxtdhCKIBMOC0IfWmpr0okokXfwKKw-PSxcDnoJrJ2v",
    codes: [
      { code: "GENSHINGIFT",   rewards: "50 Primogemas, 3 Ingenio del Héroe",       expires: "31 Mar 2026" },
      { code: "BS3DLY6VARE9",  rewards: "100 Primogemas",                           expires: "28 Mar 2026" },
      { code: "GENSHIN2024",   rewards: "60 Primogemas, 5 Mena mística",            expires: "30 Mar 2026" },
      { code: "XTNH3HMHAXH7",  rewards: "100 Primogemas, 10,000 Mora",             expires: "29 Mar 2026" },
      { code: "GIFTSUMM2024",  rewards: "80 Primogemas, 2 Ingenio del Héroe",       expires: "01 Abr 2026" },
    ],
  },

  "Honkai: Star Rail": {
    icon: "https://play-lh.googleusercontent.com/l3ENEB1tUKuM4uyHDuSn0fBiT2hKGPifmYcnBj7JKzXvMbfZBL8H8OWREJMcpXjfaw",
    codes: [
      { code: "STARRAILGIFT",  rewards: "50 Jade Estelar",                              expires: "31 Mar 2026" },
      { code: "HSR2024GIFT",   rewards: "100 Jade Estelar, 3 Guía del Viajero",         expires: "29 Mar 2026" },
      { code: "STARRAILCODE",  rewards: "80 Jade Estelar, 2 Registro de Aventuras",     expires: "28 Mar 2026" },
      { code: "PIONEERCAMP",   rewards: "60 Jade Estelar",                              expires: "30 Mar 2026" },
    ],
  },

  "Zenless Zone Zero": {
    icon: "https://play-lh.googleusercontent.com/RnpL8q_1OkjHHHUDdabPhXCVJ65tCuqOF0F5-f5kefNkfuCbCpxzj7IrH8mVsGVpOdU",
    codes: [
      { code: "ZZZGIFT2024",   rewards: "50 Policromías",                              expires: "31 Mar 2026" },
      { code: "WELCOMEZNZ",    rewards: "80 Policromías, 2 Batería de W-Engine",        expires: "28 Mar 2026" },
      { code: "ZZZEARLYGAME",  rewards: "100 Policromías, 1 Master Tape",              expires: "02 Abr 2026" },
      { code: "ZZZ27LAUNCH",   rewards: "60 Policromías, 3 Módulo de datos",           expires: "07 Abr 2026" },
    ],
  },

  "Arknights: Endfield": {
    icon: "https://play-lh.googleusercontent.com/E4-aQMifm9mHOBQm9KqJCmKIrFj8kAVGkUKQ7vNcobP1l_t8rn8V8NMK7CNRD4wFBqA",
    codes: [
      { code: "ENDFIELDSTART",  rewards: "50 Oroberyl, 2 Contrato de reclutamiento",  expires: "22 Abr 2026" },
      { code: "TALOS2LAUNCH",   rewards: "30 Oroberyl, 5,000 Créditos",               expires: "31 Mar 2026" },
      { code: "ENDFIELD11",     rewards: "60 Oroberyl, 1 Contrato de reclutamiento",  expires: "12 Abr 2026" },
    ],
  },

  "NIKKE": {
    icon: "https://play-lh.googleusercontent.com/vAWAKaKRNRniUtCTYN9oOsFPdJBtJdoq8hJsTb24ND6KJoS9DX0YBUzuqGf1BNvGLA",
    codes: [
      { code: "NIKKEGIFT",    rewards: "200 Gemas, 1 Vale de reclutamiento avanzado", expires: "09 Abr 2026" },
      { code: "2X2LOVE2026",  rewards: "100 Gemas, materiales de desarrollo",          expires: "09 Abr 2026" },
      { code: "DEVNOTE0326",  rewards: "150 Gemas",                                   expires: "01 Abr 2026" },
    ],
  },
};

// ── NOTICIAS DESTACADAS ────────────────────────────────────
// 6 noticias para el bento grid de la Home — una por juego

export const newsData = [
  {
    id: 1,
    gameId: "wuthering-waves",
    title: "Versión 3.2 en vivo — Sigrika y Qiuyuan se unen al roster de WuWa",
    tag: "Nueva versión",
    gameIconUrl: "https://play-lh.googleusercontent.com/ohBD7vN6RUU6KoD4PJgwv5Bg3HvDIjM4DvHd7NlRxRFkMJGxSEipUjexdFYJf7g1RA",
    imageUrl: "https://i.ytimg.com/vi/aMAJhkp0hlc/maxresdefault.jpg",
  },
  {
    id: 2,
    gameId: "genshin-impact",
    title: "Genshin Impact 6.4 Luna V — Varka debuta y Skirk regresa en Fase 2",
    tag: "Nueva versión",
    gameIconUrl: "https://play-lh.googleusercontent.com/L0Dn7FRoGO0Ocxb3MidlNpv_DMehL8xlEkVJcnb2MaxtdhCKIBMOC0IfWmpr0okokXfwKKw-PSxcDnoJrJ2v",
    imageUrl: "https://uploadstatic.mihoyo.com/contentweb/20240925/2024092508381918285.jpg",
  },
  {
    id: 3,
    gameId: "honkai-star-rail",
    title: "Honkai: Star Rail 4.1 — Ashveil debuta el 25 de Marzo",
    tag: "Próxima versión",
    gameIconUrl: "https://play-lh.googleusercontent.com/l3ENEB1tUKuM4uyHDuSn0fBiT2hKGPifmYcnBj7JKzXvMbfZBL8H8OWREJMcpXjfaw",
    imageUrl: "https://fastcdn.hoyoverse.com/content-v2/nap/114007/e40d44fb1f1b7d6da69090a53b5c9dd8_4600069891524753012.png",
  },
  {
    id: 4,
    gameId: "zenless-zone-zero",
    title: "ZZZ 2.7 llega el 24 de Marzo con ediciones físicas para consolas",
    tag: "Anuncio",
    gameIconUrl: "https://play-lh.googleusercontent.com/RnpL8q_1OkjHHHUDdabPhXCVJ65tCuqOF0F5-f5kefNkfuCbCpxzj7IrH8mVsGVpOdU",
    imageUrl: "https://fastcdn.hoyoverse.com/content-v2/nap/120614/7e8f6e58f2df53a6b3c9e1e9d7a4b2c1_1234567890123456789.jpg",
  },
  {
    id: 5,
    gameId: "arknights-endfield",
    title: "Arknights: Endfield 1.1 — Tangtang y Rossi en Qingbo Stockade",
    tag: "Nueva versión",
    gameIconUrl: "https://play-lh.googleusercontent.com/E4-aQMifm9mHOBQm9KqJCmKIrFj8kAVGkUKQ7vNcobP1l_t8rn8V8NMK7CNRD4wFBqA",
    imageUrl: "https://fastcdn.hoyoverse.com/content-v2/nap/114007/arknights-endfield-banner-v1.jpg",
  },
  {
    id: 6,
    gameId: "nikke",
    title: "NIKKE — Evento 2X2 LOVE y colaboración con Razer en vivo",
    tag: "Evento",
    gameIconUrl: "https://play-lh.googleusercontent.com/vAWAKaKRNRniUtCTYN9oOsFPdJBtJdoq8hJsTb24ND6KJoS9DX0YBUzuqGf1BNvGLA",
    imageUrl: "https://nikke-en.com/assets/img/news/update_march_2026.jpg",
  },
];

// ── EVENTOS ───────────────────────────────────────────────
// Eventos activos y próximos por juego

export const EVENTS_DATA = [
  {
    id: "ev-ww-1",
    gameId: "wuthering-waves",
    gameName: "Wuthering Waves",
    gameIconUrl: "https://play-lh.googleusercontent.com/ohBD7vN6RUU6KoD4PJgwv5Bg3HvDIjM4DvHd7NlRxRFkMJGxSEipUjexdFYJf7g1RA",
    title: "Tapes of Last Words",
    type: "Evento de historia",
    startDate: "2026-02-26",
    endDate: "2026-04-08",
    status: "active",
  },
  {
    id: "ev-gi-1",
    gameId: "genshin-impact",
    gameName: "Genshin Impact",
    gameIconUrl: "https://play-lh.googleusercontent.com/L0Dn7FRoGO0Ocxb3MidlNpv_DMehL8xlEkVJcnb2MaxtdhCKIBMOC0IfWmpr0okokXfwKKw-PSxcDnoJrJ2v",
    title: "Travelers' Tales",
    type: "Evento principal",
    startDate: "2026-02-25",
    endDate: "2026-04-07",
    status: "active",
  },
  {
    id: "ev-hsr-1",
    gameId: "honkai-star-rail",
    gameName: "Honkai: Star Rail",
    gameIconUrl: "https://play-lh.googleusercontent.com/l3ENEB1tUKuM4uyHDuSn0fBiT2hKGPifmYcnBj7JKzXvMbfZBL8H8OWREJMcpXjfaw",
    title: "Novaflare",
    type: "Evento de prueba de personajes",
    startDate: "2026-02-12",
    endDate: "2026-03-24",
    status: "active",
  },
  {
    id: "ev-zzz-1",
    gameId: "zenless-zone-zero",
    gameName: "Zenless Zone Zero",
    gameIconUrl: "https://play-lh.googleusercontent.com/RnpL8q_1OkjHHHUDdabPhXCVJ65tCuqOF0F5-f5kefNkfuCbCpxzj7IrH8mVsGVpOdU",
    title: "Hollow Zero — Temporada 2.6",
    type: "Contenido endgame",
    startDate: "2026-02-05",
    endDate: "2026-03-24",
    status: "active",
  },
  {
    id: "ev-ae-1",
    gameId: "arknights-endfield",
    gameName: "Arknights: Endfield",
    gameIconUrl: "https://play-lh.googleusercontent.com/E4-aQMifm9mHOBQm9KqJCmKIrFj8kAVGkUKQ7vNcobP1l_t8rn8V8NMK7CNRD4wFBqA",
    title: "Break the Siege — 10 contratos gratis",
    type: "Misión principal",
    startDate: "2026-03-12",
    endDate: "2026-05-12",
    status: "active",
  },
  {
    id: "ev-nk-1",
    gameId: "nikke",
    gameName: "NIKKE",
    gameIconUrl: "https://play-lh.googleusercontent.com/vAWAKaKRNRniUtCTYN9oOsFPdJBtJdoq8hJsTb24ND6KJoS9DX0YBUzuqGf1BNvGLA",
    title: "2X2 LOVE — Dating Sim Story Event",
    type: "Evento de historia",
    startDate: "2026-03-19",
    endDate: "2026-04-09",
    status: "active",
  },
];
