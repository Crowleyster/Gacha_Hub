...
import {
    Filter, Check, ChevronDown, RotateCcw,
...
/* ─────────────────────────────────────────────
   JuegosClient Component
───────────────────────────────────────────── */
export default function JuegosClient({ initialGamesData }) {
    const gamesList = useMemo(() => initialGamesData, [initialGamesData]);
    ...
    const sortFn = useCallback((a, b, mode) => {
        ...
        if (mode === 'Nuevos') {
            const dateA = new Date(a.releaseDate?.pc || a.releaseDate?.android || a.releaseDate || 0);
            const dateB = new Date(b.releaseDate?.pc || b.releaseDate?.android || b.releaseDate || 0);
            return dateB - dateA;
        }
        return 0;
    }, []);
