...
───────────────────────────────────────────── */
function FiltersContent({ filters, updateFilter, hasActiveFilters, clearFilters, allTypes, gameOptions, allGames }) {
    return (
...
export default function EventosClient({ initialEvents, initialGames }) {
    // 1. Data & State
    const allEvents = useMemo(() => initialEvents, [initialEvents]);
    const allGames = useMemo(() => initialGames, [initialGames]);

    const allTypes = useMemo(() => Array.from(new Set(allEvents.map(e => e.type))).sort(), [allEvents]);
    const gameOptions = useMemo(() => allGames.map(g => g.id), [allGames]);
